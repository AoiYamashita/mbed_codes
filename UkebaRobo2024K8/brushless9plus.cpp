...
BrushLess::BrushLess(PinName CAN_RX, PinName CAN_TX) : can(CAN_RX, CAN_TX, 1000000),abs_degree{0,0,0,0,0,0,0,0}{
}
...
void BrushLess::CanIntr() {
    ...
    for (int i = 0; i < 8; i++) {
        if (msg.id == CAN_RECEIVE_ID[i]) {
            ...
            if(abs(deffer_0) < abs(deffer_1)) abs_degree[i] += deffer_0;
            else abs_degree[i] += deffer_1;
